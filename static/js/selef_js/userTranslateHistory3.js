document.addEventListener("DOMContentLoaded", function () {
    ...
    submitButton.addEventListener("click", function () {
        ...
        fetch("/user_translate_history/3", {
            method: "POST",
            headers: {
    ...
    function updateTableWithData(dataList, totalPages, currentPage) {
        ...
        dataList.forEach((item, index) => {
            ...
            updateForm.action = "/update_translation_item/3";
            let updateButton = createButton("🤍", "btn btn-primary btn-xs");
            console.log(item[4]);
            if (item[4] == 0) {
                updateButton = createButton("🤍", "btn btn-primary btn-xs");
            } else if (item[4] == 1) {
                updateButton = createButton("♥推荐♥", "btn btn-primary btn-xs");
            }

            const deleteForm = document.createElement("form");
            deleteForm.method = "POST";
            deleteForm.action = "/delete_translation_list/3";
